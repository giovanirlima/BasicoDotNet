...
public class UpdateAvisoHandlerTests
{
    ...
    public UpdateAvisoHandlerTests()
    {
        ...
        _handler = new UpdateAvisoHandler(services.BuildServiceProvider());
    }
    ...
    public async Task Should_Update_Aviso()
    {
        ...
        request.SetIdProperty(1);

        _repositoryMock.Setup(r => r.ObterAvisoPorIdAsync(1, It.IsAny<TrackingBehavior>(), It.IsAny<CancellationToken>()))
        ...
        _repositoryMock.Setup(r => r.UpdateAvisoAsync(request.Id, request.Mensagem, It.IsAny<CancellationToken>())).Returns(Task.CompletedTask);

        var result = await _handler.Handle(request, default);

        _repositoryMock.Verify(r => r.UpdateAvisoAsync(request.Id, request.Mensagem, It.IsAny<CancellationToken>()), Times.Once);
